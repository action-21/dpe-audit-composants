...
export namespace Mur {
  export interface Mur extends ParoiOpaque {
  ...
  export const toString = (value: Mur.Mur): string => {
    let text: string = value.description

    if (value.type_structure) {
      text += `\n${typeStructureToString(value.type_structure)}`

      if (value.epaisseur_structure) {
        text += ` - ${value.epaisseur_structure} mm`
      }
    }
    if (value.type_doublage) {
      text += `\n${typeDoublageToString(value.type_doublage)}`
    }

    text += `\n${isolationToString(value.isolation)}`
    return text
  }
}

export namespace PlancherBas {
  export interface PlancherBas extends ParoiOpaque {
  ...
  export const toString = (value: PlancherBas.PlancherBas): string => {
    let text: string = value.description
...
export namespace PlancherHaut {
  export interface PlancherHaut extends ParoiOpaque {
  ...
  export const toString = (value: PlancherHaut.PlancherHaut): string => {
    let text: string = value.description
...
export namespace Baie {
  export interface Baie extends Paroi {
  ...
  export const toString = (value: Baie.Baie): string => {
    let text: string = value.description

    if (value.vitrage?.type_vitrage) {
      text += value.vitrage.type_vitrage
        ? `\n${typeVitrageToString(value.vitrage.type_vitrage)}`
        : `\nVitrage inconnu`

      if (value.vitrage.nature_lame) {
        text += ` - ${natureLameToString(value.vitrage.nature_lame)}`
      }
      if (value.vitrage.epaisseur_lame) {
        text += ` - ${value.vitrage.epaisseur_lame} mm`
      }
      if (value.vitrage.survitrage) {
        text += value.vitrage.survitrage.type_survitrage
          ? `\n${typeSurvitrageToString(
              value.vitrage.survitrage.type_survitrage
            )}`
          : `\nSurvitrage inconnu`

        if (value.vitrage.survitrage.epaisseur_lame) {
          text += ` - ${value.vitrage.survitrage.epaisseur_lame} mm`
        }
      }
    }
...
export namespace Porte {
  export interface Porte extends Paroi {
    type_pose: TypePose
  ...
  export const toString = (value: Porte.Porte): string => {
    let text: string = value.description

    if (value.vitrage.taux_vitrage > 0) {
      text += value.vitrage.type_vitrage
        ? `\n${typeVitrageToString(value.vitrage.type_vitrage)}`
        : `\nVitrage inconnue`
    }
...
export namespace PontThermique {
  export interface PontThermique {
    ...
    data: PontThermique.PontThermiqueData
  }
  ...
  export const toString = (value: PontThermique.PontThermique): string => {
    let text = `${typeLiaisonToString(value.liaison.type_liaison)}`
    text += `\n${value.description}`
